...
print("--- APPROACH 9: Combined Submissions (Train + Val + Test) ---")
...
# Apply identical logic to TRAIN logic for combined submission
vp_tr_ens = get_ensemble_probs(X_tr)
hr_flags_tr = hard_rules_layer(train).values
preds_tr = (vp_tr_ens > best_t).astype(bool)
preds_tr |= hr_flags_tr
remove_mask_tr = (vp_tr_ens < 0.3) & (~hr_flags_tr)
remove_mask_tr |= (train['call_duration'].fillna(0) > 600) & (train['answered_count'].fillna(0) == 1)
preds_tr[remove_mask_tr] = False

# Create individual dataframes and concatenate
sub_tr = pd.DataFrame({'call_id': train['call_id'], 'has_ticket': preds_tr.astype(int)})
sub_va = pd.DataFrame({'call_id': val['call_id'], 'has_ticket': preds_val.astype(int)})
sub_te = pd.DataFrame({'call_id': test['call_id'], 'has_ticket': preds_test.astype(int)})

sub_v9 = pd.concat([sub_tr, sub_va, sub_te], ignore_index=True)
sub_v9.to_csv('REAL/submission_v9.csv', index=False)
print(f"SUCCESS! Created submission_v9.csv with {len(sub_v9)} total rows.")
print(f"Total Combined Flags: {sub_v9['has_ticket'].sum()} (Train:{preds_tr.sum()} + Val:{preds_val.sum()} + Test:{preds_test.sum()})")

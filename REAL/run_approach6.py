...
# To simulate LLM flags quickly for train, we'll try and just train without it, then add rule logic
...
for t in np.arange(0.3, 0.95, 0.02):
    preds_a6 = (vp_ens > t).astype(bool)
    
    # Approach 3 Logic: Combine everything
    preds_a6 |= hr_flags
    
    # Remove from ML flags if ML prob < 0.3 AND no hard rule fires
    remove_mask = (vp_ens < 0.3) & (~hr_flags)
    preds_a6[remove_mask] = False
    
    f1 = f1_score(y_va, preds_a6.astype(int))
    if f1 > best_f1_a6:
        best_f1_a6 = f1
        best_t_a6 = t
...
f1 = f1_score(y_va, preds_best_a6)
prec = precision_score(y_va, preds_best_a6, zero_division=0)
rec = recall_score(y_va, preds_best_a6)

tp = (y_va.astype(bool) & preds_best_a6.astype(bool)).sum()
fp = (~y_va.astype(bool) & preds_best_a6.astype(bool)).sum()
fn = (y_va.astype(bool) & ~preds_best_a6.astype(bool)).sum()

print("--- APPROACH 6: Combine Everything ---")
print(f"Val F1: {f1:.4f} | Prec: {prec:.4f} | Rec: {rec:.4f} (thresh={best_t_a6:.2f})")
print(f"TP={tp} FP={fp} FN={fn} Flagged={preds_best_a6.sum()}")

sub_a6 = pd.DataFrame({'call_id': val['call_id'], 'has_ticket': preds_best_a6})
sub_a6.to_csv('REAL/submission_v6_val.csv', index=False)

if f1 > 0.50:
    print("SUCCESS! Approach 6 exceeded 0.50 Val F1.")
